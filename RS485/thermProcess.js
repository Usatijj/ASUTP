...
startStop(false,(err)=>{
  // перед началом работы останавливаем все терморегуляторы
  if (err) {
...
function testTemperature(tT,cb){
  // определяет конец цикла набора температурі(разогрева) по температурам на всех ТРП
  // -----   tT = task temperature  --------------------------
  // возвращает true, если разогрев закончился, иначе false
  // ----------логгер
  ...
  if (forseStop) {
   ...
   let state =  (data.T1.value >= tT-dT) &
                ...
                (data.T1.value >= tT-dT);
   if (state) {
          // цикл разогрева закончился
          cb(null,true);
          return
          };
   // цикл разогрева продолжается
   return cb(null,false)
 } //testTemperature


function cloneObj(arg){
  // создает копию объекта для его изменения
  let newTask={};
  for (let key in arg) {
      newTask[key]=arg[key];
  }//for
...
function startProgram(args){
  ...
  if (mode | forseStop) {
   ...
   // сохраняем в глобальной зоне модуля настройки программы
   tasks=args;
   // клонируем первый шаг и изменяем в нем время удержания на "1:30"
